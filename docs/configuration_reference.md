...
| `fileType` | The file type to save the file as. Allowed values are `json`, `xlsx`, and `sqlite`. When `sqlite` is used, the data is written into a local SQLite database that accumulates history across runs. | Yes | `json` |
