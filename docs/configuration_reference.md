...
| `fileType` | The file type to save the file as. Allowed values are `json`, `xlsx`, and `sqlite`. | Yes | `json` |
| `fileNamePrefix` | The prefix to add to the file name. | No |`""` |
| `serverMode` | Enables server-mode naming. Non-sqlite file types get a timestamp suffix; `sqlite` keeps a stable file name so data can be merged over time. | No | `false` |
